#!/usr/bin/env python3
"""MundMaus Enclosure v5.6 — Sensor to +X wall, hold-down fix.

v5.6 changes from v5.5:
  - Pressure sensor relocated from +Y wall to +X wall (flat mount, barb through +X)
  - ESP32 hold-down wall shortened by 1mm (HOLD_DOWN_CLEARANCE) so lid closes properly
"""
...
def render_pngs(base: cq.Workplane, lid: cq.Workplane, outdir: Path) -> None:
    views = [
        RenderView("mundmaus_v56_assembly.png", (1.0, -1.0, 0.8), True),
        RenderView("mundmaus_v56_top.png", (0.0, 0.0, 1.0), True),
        RenderView("mundmaus_v56_side.png", (0.0, 1.0, 0.0), False),
        RenderView("mundmaus_v56_back.png", (-1.0, 0.0, 0.0), False),
    ]
...
def write_report(report_path: Path) -> None:
    report = textwrap.dedent(
        ...
        v5.6 changes from v5.5:
        - **Pressure sensor relocated** from +Y wall to +X wall (flat mount, barb through +X)
        - **ESP32 hold-down wall** shortened by {HOLD_DOWN_CLEARANCE:.0f}mm clearance so lid closes properly
        ...
        | Feature | v5.5 | v5.6 |
        |---|---|---|
        | Pressure sensor mount | +Y wall shelf at X=42 | +X wall flat mount, shelf at Z={PRES_Z_BOT:.0f} |
        | Barb hole | +Y wall, D3.0mm | +X wall, D{PRES_BARB_HOLE_D:.1f}mm (press-fit) |
        | Hose routing | mouthpiece -> +Y barb | mouthpiece -> +X barb |
        | Hold-down wall | touches ESP32 PCB (0mm gap) | {HOLD_DOWN_CLEARANCE:.0f}mm clearance |
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MundMaus v5.6 enclosure")
    ...
    base_path = outdir / "mundmaus_v56_base.stl"
    lid_path = outdir / "mundmaus_v56_lid.stl"
    report_path = outdir / "mundmaus_v56_report.md"

    print(f"MundMaus v5.6 — CadQuery {cq.__version__}")
    print(f"  External: {EXT_X:.1f} x {EXT_Y:.1f} x {EXT_H_BASE + EXT_H_LID:.1f} mm")
    print(f"  Layout: Mount(-X) → Joy(X={JOY_POS_X:.0f}) → ESP32(X={ESP_POS_X:.0f}) → Sensor(+X wall)")
