#!/usr/bin/env python3
"""MundMaus Enclosure v5.3 — USB-C Panel Mount Redesign

CadQuery parametric design for FDM (Bambu Lab P1S, PETG)

v5.3 Changes vs v5.2:
  1. USB cable exit replaced by USB-C panel-mount bulkhead hole (8.5mm)
     on +Y wall at X=45, Z=14
  2. Nut recess on inside of +Y wall (12mm dia, 2mm deep)
  3. Two cable clips on floor for internal cable strain relief
  4. USB cable notch removed from base and lid (tube notch kept)
  5. Old rectangular USB opening removed

Layout (top view, patient lies below at -Y):
  +Y wall (TOP — USB-C panel mount)
  +------------------------------------------------------+
...
# USB-C Panel Mount (Bulkhead, +Y wall) — v5.3 replaces old USB cable exit
USBC_MOUNT_HOLE_D = 8.5      # Bore for threaded cylinder
USBC_MOUNT_X = 45.0           # X position on +Y wall (same as old USB exit)
USBC_MOUNT_Z = 14.0           # Z position (center of wall height)
USBC_NUT_D = 12.0             # Counter-nut diameter
USBC_NUT_RECESS = 2.0         # Recess depth for nut (inside wall)

# Cable Clips (internal strain relief)
CLIP_H, CLIP_W, CLIP_OPENING = 5.0, 3.0, 4.0
CLIP_WALL_T = 1.5             # Clip wall thickness
CLIP1_X, CLIP1_Y = 52.0, 5.0   # Near ESP32 USB end
CLIP2_X, CLIP2_Y = 45.0, 18.0  # Near panel mount

# Cable Exit Notches (tube only in v5.3)
...
def _cut_panel_mount_hole(base):
    """USB-C panel-mount bulkhead hole on +Y wall.

    8.5mm bore through 3mm wall, with 12mm x 2mm nut recess on inside.
    Position: X=45, Z=14 from base bottom.
    """
    wall_outer_y = EXT_Y / 2  # +Y outer face
    hole_z = FLOOR_T + USBC_MOUNT_Z  # absolute Z (floor offset + height above floor)

    # Through-hole (8.5mm) — cut through full wall thickness
    bore = cq.Workplane("XZ").workplane(offset=-wall_outer_y - 0.01) \
        .center(USBC_MOUNT_X, hole_z) \
        .circle(USBC_MOUNT_HOLE_D / 2).extrude(WALL + 0.02)
    base = base.cut(bore)

    # Nut recess on inside face (12mm dia, 2mm deep)
    wall_inner_y = wall_outer_y - WALL  # +Y inner face
    recess = cq.Workplane("XZ").workplane(offset=-wall_inner_y + 0.01) \
        .center(USBC_MOUNT_X, hole_z) \
        .circle(USBC_NUT_D / 2).extrude(USBC_NUT_RECESS + 0.01)
...
def _cut_cable_notches(base):
    """Tube cable notch on -X wall only. USB notch removed in v5.3."""
    base_top = EXT_H_BASE
...
def _add_cable_clips(base):
    """Two C-shaped cable clips on the floor for internal USB cable strain relief.

    Clip 1: Near ESP32 USB end (critical — catches cable pull force)
    Clip 2: Near panel mount (cable routing guide)

    Each clip is a cylinder with a slot cut out to form a C-shape,
    open toward +Y so cable can be pressed in from above.
    """
    for clip_x, clip_y in [(CLIP1_X, CLIP1_Y), (CLIP2_X, CLIP2_Y)]:
        outer_r = (CLIP_OPENING + 2 * CLIP_WALL_T) / 2  # outer radius
        inner_r = CLIP_OPENING / 2  # inner radius for cable

        # Outer cylinder
        outer = cq.Workplane("XY").workplane(offset=FLOOR_T) \
            .center(clip_x, clip_y) \
            .circle(outer_r).extrude(CLIP_H)

        # Inner bore (cable channel)
        inner = cq.Workplane("XY").workplane(offset=FLOOR_T - 0.01) \
            .center(clip_x, clip_y) \
            .circle(inner_r).extrude(CLIP_H + 0.02)

        clip = outer.cut(inner)

        # C-opening: slot toward +Y for cable insertion
        slot = cq.Workplane("XY").workplane(offset=FLOOR_T - 0.01) \
            .center(clip_x, clip_y + outer_r) \
            .rect(CLIP_OPENING - 0.5, outer_r * 2).extrude(CLIP_H + 0.02)
        clip = clip.cut(slot)
...
def make_base():
    ...
    for fn in [_add_screw_bosses, _add_esp_cradle, _add_joystick_platform,
               _add_pressure_sensor_ledge, _cut_panel_mount_hole, _cut_tube_feedthrough,
               _cut_cable_notches, _add_cable_clips, _add_mic_mount, _cut_vent_slots]:
        base = fn(base)
    return base
...
def make_lid():
    ...
    # Cable notch: tube on -X wall only (USB notch removed in v5.3)
    ...
    try:
        label = cq.Workplane("XY").workplane(offset=EXT_H_LID).center(0, 14) \
            .text("MundMaus v5.3", 7, 0.6, font="Liberation Sans:Bold")
        lid = lid.union(label)
...
def main():
    parser = argparse.ArgumentParser(description="MundMaus Enclosure v5.3")
    ...
    print(f"MundMaus v5.3 Enclosure — CadQuery {cq.__version__}")
    ...
    print(f"  USB-C panel mount: +Y wall at X={USBC_MOUNT_X}, Z={USBC_MOUNT_Z}, hole {USBC_MOUNT_HOLE_D}mm")
    print(f"  Stick protrusion: {STICK_PROTRUSION:.1f} mm (target: >=3mm)\n")

    if args.part in ("base", "both"):
        print("Building base...")
        export_stl(make_base(), str(outdir / "mundmaus_v53_base.stl"))
    if args.part in ("lid", "both"):
        print("Building lid...")
        lid = make_lid()
        lid_print = lid.rotateAboutCenter((1, 0, 0), 180).translate((0, 0, EXT_H_LID))
        export_stl(lid_print, str(outdir / "mundmaus_v53_lid.stl"))
