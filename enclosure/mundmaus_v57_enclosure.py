#!/usr/bin/env python3
"""MundMaus Enclosure v5.7 — Lid-only fix (lip reduction + ridge strengthening).

v5.7 changes from v5.6 (LID ONLY, base unchanged):
  - LIP_H 4.0 → 3.0 mm: fixes collision between lid inner lip and sensor PCB top (z=25.5).
    Lip zone moves from z=26-30 to z=27-30, giving 1.5mm clearance instead of negative.
  - RIDGE_H 0.5 → 0.6 mm: 20% more detent retention force (interference with GROOVE_D=0.55
    creates small preload, PETG lip flex absorbs it).
  - RIDGE_Z 2.0 → 1.0 mm: realigned with existing base grooves after LIP_H reduction.
    Groove Z in assembly coords stays at 28mm: EXT_H_BASE - LIP_H + RIDGE_Z = 30 - 3 + 1 = 28.
"""
...
RIDGE_H = 0.6        # ridge height — 0.5→0.6 for 20% more retention (0.05mm interference with GROOVE_D=0.55)
RIDGE_W = 0.8        # ridge width (along Z axis of lip)
RIDGE_Z = 1.0        # ridge position: mm from lip bottom edge (2.0→1.0 realigns with base grooves after LIP_H reduction)
...
def render_pngs(base: cq.Workplane, lid: cq.Workplane, outdir: Path) -> None:
    views = [
        RenderView("mundmaus_v57_assembly.png", (1.0, -1.0, 0.8), True),
        RenderView("mundmaus_v57_top.png", (0.0, 0.0, 1.0), True),
        RenderView("mundmaus_v57_side.png", (0.0, 1.0, 0.0), False),
        RenderView("mundmaus_v57_back.png", (-1.0, 0.0, 0.0), False),
    ]
...
def write_report(report_path: Path) -> None:
    report = textwrap.dedent(
        ...
        ## Summary
        v5.7 changes from v5.6 (LID ONLY, base unchanged):
        - **LIP_H {LIP_H:.1f} mm** (was 4.0): fixes collision between lid inner lip and sensor PCB top at z=25.5.
          Lip zone now {LIP_ZONE_BOTTOM:.1f}–{EXT_H_BASE:.0f}, giving {LIP_ZONE_BOTTOM - 25.5:.1f}mm clearance above the sensor.
        - **RIDGE_H {RIDGE_H:.1f} mm** (was 0.5): 20% more detent retention force.
        - **RIDGE_Z {RIDGE_Z:.1f} mm** (was 2.0): realigned so ridges still meet existing base grooves at z=28.

        Ridge/groove alignment (assembly coords):
        - Base groove z = EXT_H_BASE - 4.0 + 2.0 = 28 mm (v5.6 base, UNCHANGED)
        - Lid ridge z  = EXT_H_BASE - {LIP_H:.1f} + {RIDGE_Z:.1f} = {EXT_H_BASE - LIP_H + RIDGE_Z:.1f} mm ✓
        ...
        | Item | Value |
        |---|---:|
        | Lip zone bottom (was 26.0) | {LIP_ZONE_BOTTOM:.1f} mm |
        | Sensor PCB top to lip zone | {LIP_ZONE_BOTTOM - PRES_Z_TOP:.1f} mm |
        | Ridge height (was 0.5) | {RIDGE_H:.1f} mm |
        | Ridge interference with groove (GROOVE_D={GROOVE_D:.2f}) | {RIDGE_H - GROOVE_D:+.2f} mm |
        | Ridge Z in assembly coords | {EXT_H_BASE - LIP_H + RIDGE_Z:.1f} mm (matches base groove) |
        ...
        | Sensor shelf inner edge X | {PRES_SHELF_X_START:.1f} mm |
        ...
        | Lid attachment | detent ridges (7 points, {RIDGE_H:.1f}mm) |
        ## Changes vs v5.6
        | Feature | v5.6 | v5.7 |
        |---|---|---|
        | LIP_H | 4.0 mm | {LIP_H:.1f} mm |
        | RIDGE_H | 0.5 mm | {RIDGE_H:.1f} mm |
        | RIDGE_Z | 2.0 mm | {RIDGE_Z:.1f} mm |
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MundMaus v5.7 enclosure (lid-only fix)")
    ...
    base_path = outdir / "mundmaus_v57_base.stl"
    lid_path = outdir / "mundmaus_v57_lid.stl"
    report_path = outdir / "mundmaus_v57_report.md"

    print(f"MundMaus v5.7 — CadQuery {cq.__version__}")
    print(f"  External: {EXT_X:.1f} x {EXT_Y:.1f} x {EXT_H_BASE + EXT_H_LID:.1f} mm")
    print(f"  Lid-only fix: LIP_H={LIP_H}, RIDGE_H={RIDGE_H}, RIDGE_Z={RIDGE_Z}")
    print(f"  Lip zone bottom: {LIP_ZONE_BOTTOM:.1f} mm (was 26.0)")
