...
test.describe('Portal (ESP32 root)', () => {
  ...
  test('7. keyboard nav: arrow keys move highlight between game buttons', async ({ page }) => {
    // Use inline style (el.style.boxShadow) not computed style:
    // - computed is unreliable during CSS transition (.g has transition:all .2s)
    // - _navIdx is declared with `let` so not accessible via window._navIdx
    // - _navUpdate() sets ALL items' inline boxShadow: highlighted=glow, others='none'
    async function getHighlightedIndex(): Promise<number> {
      return page.evaluate(() => {
        const items = document.querySelectorAll<HTMLElement>('.g');
        for (let i = 0; i < items.length; i++) {
          const s = items[i].style.boxShadow;
          if (s && s !== 'none') return i;
        }
        return -1;
      });
    }
